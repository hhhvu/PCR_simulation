...
class FusionModel(nn.Module):
    def __init__(self, input_size, hidden_size, latent_dim, sequence_length, num_layers=5):
        ...
        # TODO change to true
        self.effnet = models.efficientnet_v2_l(pretrained=False)
...
if __name__ == "__main__":
    ...
    train_dataset = SequenceDataset(curve_dict, target_df,train=True, sequence_len=40,
                                         mean=norm_mean, std = norm_std)
    val_dataset = SequenceDataset(curve_dict, target_df, train=False, sequence_len=40,
                                       mean=norm_mean, std = norm_std)
    ...
    for epoch in tqdm(range(num_epoch)):  # Choose the number of epochs
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'output/fusion_model/best_model_no_pretrain.pth')
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('output/fusion_model/loss_curve_no_pretrain.png')
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('AUC')
    plt.savefig('output/fusion_model/auc_across_epochs_curve_no_pretrain.png')